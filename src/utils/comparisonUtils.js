export function getComparisonResult(comparison) {
    ...
    const normalizedChoice = isABFormat 
        ? (comparison.choice === "A" ? "1" : "2")
        : comparison.choice;
    
    const choice = Number(normalizedChoice);
    
    return {
        moreValuableProject: choice === 1 ? comparison.itemAName : comparison.itemBName,
        lessValuableProject: choice === 1 ? comparison.itemBName : comparison.itemAName,
